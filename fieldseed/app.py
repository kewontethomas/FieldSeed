...
class FieldSeedApp(tk.Tk):
    ...
    def active_agent_tick(self):
        try:
            r = run_health_check()
            self.status_var.set(f"Active agent: {r['checked']} files checked, {len(r['failures'])} failures, {len(r['open_tickets'])} open ticket(s)")
        except Exception as e:
            self.status_var.set(f"Active agent issue: {e}")
        self.after(300000, self.active_agent_tick)
    ...
    def page_growth(self):
        self.summary.config(text="FieldSeed self-awareness center: it scans, creates repair candidates automatically, asks for approval, tests repairs, and keeps rollback records.")
        p = self.scroll_page()
        p.grid_columnconfigure(0, weight=1)
        ...
        actions.grid(row=2, column=0, sticky="w", padx=14, pady=(0, 12))
        self.button(actions, "Run Visual Self-Inspection", self.run_visual_self_inspection_ui, "primary").pack(side="left", padx=4)
