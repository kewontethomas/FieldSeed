...
def self_check_record(name, status, details="", recommendation=""):
    con = connect()
    con.execute("INSERT INTO self_checks(created_at, check_name, status, details, recommendation) VALUES (?, ?, ?, ?, ?)",
...
def add_improvement(source, issue, evidence="", suggested_fix=""):
    con = connect()
    con.execute("INSERT INTO improvement_queue(created_at, source, issue, evidence, suggested_fix) VALUES (?, ?, ?, ?, ?)",
